...
def model_cheat_sheet() -> list[str]:
    return [
        ...
        "- agent_send_peer [--to <alias>|--to <a>,<b>|--all] --watchdog <sec> 'request' : add watchdog wakes using the same <sec> per phase: first if bridge delivery/submission stalls, then again after prompt delivery if the peer does not answer. A request may therefore wait up to two phase intervals before a response-phase wake (for example default 300s delivery + 300s response). Put options before the inline body, or use --stdin for complex bodies. Request only. --watchdog requires auto-return; --no-auto-return requests do not get watchdogs, and --watchdog 0 disables the default. Default delay is set via env AGENT_BRIDGE_DEFAULT_WATCHDOG_SEC (300).",
...
def probe_prompt(mode: str, probe_id: str, alias: str, peers: str) -> str:
    ...
    return (
        ...
        "\n"
        "Waiting / self-wake:\n"
        "  --watchdog <sec> (on a request)                      - wake yourself using the same <sec> per phase: first if bridge delivery/submission stalls, then after prompt delivery if no reply arrives. A request may wait up to two phase intervals before a response-phase wake (for example default 300s delivery + 300s response). Requires auto-return; --no-auto-return requests get no watchdog, and --watchdog 0 disables the default.\n"
