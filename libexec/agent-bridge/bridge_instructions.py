...
def model_cheat_sheet() -> list[str]:
    return [
        ...
        "- agent_interrupt_peer <alias> : use when you sent the wrong prompt or a peer is stuck. ESC + cancel the active message; it is removed, not requeued. No default hold: queued or newly sent corrections may deliver after ESC succeeds. If the peer is already past the inflight phase, interrupt can be a no-op: the response and queued follow-ups still flow. Identifiable late output from the cancelled turn is ignored.",
        "- agent_interrupt_peer <alias> --clear-hold : clear legacy held_interrupt residue without sending ESC. Normally unnecessary post-v1.5 because queued/new corrections deliver without --clear-hold. UNSAFE if the peer might still be running a turn — late Stop events will be ignored or treated as stale. Inspect with --status first.",
...
def probe_prompt(mode: str, probe_id: str, alias: str, peers: str) -> str:
    ...
    return (
        ...
        "  agent_interrupt_peer <alias>                         - use for a wrong prompt or stuck peer; ESC + cancel active message. If already past inflight phase, interrupt can be a no-op: the response and queued follow-ups still flow.\n"
