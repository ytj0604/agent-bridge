...
def model_cheat_sheet() -> list[str]:
    return [
        ...
        "- agent_wait_status [--summary|--why] : show your own outstanding waits, alarms, watchdogs, and pending inbound bridge queue as JSON, or use --why for a human-readable diagnosis. Use only for a human-prompted check, after a watchdog wake, or suspected bridge-state debugging; do not poll for progress or replace waiting for [bridge:*] prompts. Use agent_view_peer for peer pane debugging.",
        ...
        "- agent_clear_peer <alias>[,<alias>...] | agent_clear_peer --to <alias>[,<alias>...] | agent_clear_peer --all : clear peer model context with controlled /clear probes. Single-target self-clear is deferred until your current turn ends. Multi-clear is guard-atomic but sequential best-effort; --force is single-target only and self-clear must be the only target. Single-target --force clears soft blockers such as pending/pre-active inbound rows, target-owned alarms, and target-originated request return routes; active/post-pane-touch work remains a hard blocker.",
...
def probe_prompt(mode: str, probe_id: str, alias: str, peers: str) -> str:
    ...
    return (
        ...
        "\n"
        "First action: load the 'agent-bridge' skill now (it carries the full operating guide and stays anchored across compaction). "
        "Claude Code: invoke the Skill tool with skill name 'agent-bridge'. "
        "Codex: invoke the agent-bridge skill. "
        "If the skill is unavailable in your environment, run agent_list_peers instead.\n"
        "\n"
        "Sending:\n"
