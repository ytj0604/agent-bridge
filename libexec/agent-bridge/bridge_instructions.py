...
def model_cheat_sheet() -> list[str]:
    return [
        ...
        "Waking and timing:",
        "- After sending a request, end your turn. The peer's reply arrives automatically as a [bridge:*] result. With the default watchdog enabled you also get a [bridge:watchdog] notice if the peer takes too long.",
...
def probe_prompt(mode: str, probe_id: str, alias: str, peers: str) -> str:
    ...
    return (
        ...
        "\n"
        "Behavior rules:\n"
        "  - After sending a request, END YOUR TURN. The reply arrives later as a [bridge:*] result, same delivery path as a user prompt; do not loop or poll.\n"
