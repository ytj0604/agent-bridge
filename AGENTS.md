...
Start at `bin/bridge_run.sh` and `libexec/agent-bridge/bridge_attach.py`: they discover Claude/Codex panes, choose aliases, create room state, and ensure a daemon. Runtime paths are centralized in `bridge_paths.py`; participant/session validation is in `bridge_participants.py` and `bridge_identity.py`.
